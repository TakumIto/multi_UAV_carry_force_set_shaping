function visualize_platform(N, alphas, betas, gammas, scale)
%VISUALIZE_PLATFORM この関数の概要をここに記述
%   詳細説明をここに記述

% set(gcf,'Renderer','OpenGL');
% left=100; bottom=100; width=823; height=784;
% set(gcf,'Position',[left bottom width height],'Color',[1,1,1]);
plot3(0,0,0);hold on;
axis vis3d; grid on; %axis off; 
% xlabel('x');
% ylabel('y');
% zlabel('z');
...
payload = patch('Vertices',p_shape,'Faces',cube_face,'FaceVertexCData',hsv(1),'FaceColor','#696969','FaceAlpha',0.2);
...
for i=1:N
    ...
    directions(i) = plot3(0,0,0,'r','LineWidth',3);
end
for i=1:N
...
for i=1:N
    ...
    agent_arm = [size_p(1)/2/l*(pos_ai*R_p'+pos_p);pos_ai*R_p'+pos_p];
